      ...
      prompt: {
        ...
        nodes: [
          ...
          },
          {
            ...
            widgets_values: ['NewRealityXL ❗ All-In-One Photographic/newrealityxlAllInOne_21.safetensors'],
          },
          {
            id: 7,
            type: 'CLIPTextEncode',
...
export const OldWorkflowEditorPage = () => {
  const [currentPrompts, setCurrentPrompts] = useState([]);
  const iframeRef = useRef<any>(null);
  // const handelClick = () => window.postMessage(exampleEvent);

  useEffect(() => {
    const handleMessage = (event: any) => {
      const e = event as IframeEvent<GotPromptData>;
      console.log(e);
      // if (event.origin !== IFRAME_URL) return;

      if (e.data.type === 'got_prompt') {
        console.log('HERE!', e.data.data.workflow);
        // console.log('e.data.workflow', e.data.data.workflow);
        setCurrentPrompts((p) => [...p, e.data.data.workflow] as any);
      }
  ...
  const loadPrompt = () => {
    sendMessageToIframe(exampleEvent);
  };
  ...
  const sendLoadPromptMessage = (prompt: any) => {
    const msg = {
      internal: {
        type: 'load_prompt',
        data: {
  ...
  const loadSelectedPrompt = (index: number) => {
    console.log('currentPrompts[index]', currentPrompts[index]);
    sendLoadPromptMessage(currentPrompts[index]);
  };
  ...
  return (
    <div>
      ...
      <div className="flex">
        <iframe ref={iframeRef} src={IFRAME_URL} width="1200" height="800" title="Local HTML Content"></iframe>
        <div>
          <h1 className="text-xl">Saved prompts:</h1>
          {currentPrompts.map((_p, i) => (
            <p key={i} onClick={() => loadSelectedPrompt(i)}>
              prompt {i}
            </p>
          ))}
        </div>
