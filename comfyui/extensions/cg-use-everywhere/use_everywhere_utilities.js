class Logger {
  ...
  static trace(message, array, node) {
    if (Logger.TRACE) {
      if (node) {
        console.log(`TRACE (${node.id}) : ${message}`);
      } else {
        console.log(`TRACE : ${message}`);
      }
      if (array && Logger.LEVEL >= Logger.INFORMATION)
        for (var i = 0; i < array.length; i++) {
...
function handle_bypass(original_link, type) {
  ...
  while (node_is_bypassed(parent)) {
    ...
    var link_id;
    if (parent?.inputs[link.origin_slot]?.type == type)
      link_id = parent.inputs[link.origin_slot].link; // try matching number first
