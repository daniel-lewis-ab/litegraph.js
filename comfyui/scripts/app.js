...
import { api } from '/patch/api-rewrite.js';
...
export class ComfyApp {
  ...
  async loadGraphData(graphData, clean = true) {
    ...
    // Check to make sure that a graph has been passed through, if nodes is undefined, it has not been passed correctly
    if (!graphData || graphData.nodes === undefined) {
  ...
  #formatExecutionError(error) {
    try {
      if (error == null)
        return '(Unknown Error #2060)';
      else {
        const traceback = error.traceback?.join('');
        return `Error occurred when executing ${error.node_type}:\n\n${error.exception_message}\n\n${traceback}`;
      }
    } catch(err2) {
      return `Error: ${error}`;
    }
