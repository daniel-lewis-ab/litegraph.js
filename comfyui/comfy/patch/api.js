...
class ComfyApiPatch extends EventTarget {
  ...
  i = (t) => {
    const e = new DataView(t).getUint32(0),
      s = t.slice(4);
    let a;
    if ((e === 1 ? (a = "image/jpeg") : e === 2 && (a = "image/png"), !a))
      return;
    ...
    if (
      ...
      if (e.type === "load_prompt" && e.data && e.data.prompt) {
        const s = () => {
          ...
          e.data.read_only
            ? ((this.readOnly = !0), this.n())
            : setInterval(this.p, m),
            this.s(),
            this.d();
        };
        this.app
      ...
      if (
        (e.type === "clear" && (this.app.clean(), this.app.graph.clear()),
          e.type === "undo")
      ) {
  ...
  n = () => {
    ...
    Object.assign(e, {
      ...
      Array.from(document.querySelectorAll('[type="file"]')).forEach((s) =>
        s.remove(),
      ),
      Array.from(document.querySelectorAll(".comfy-modal")).forEach((s) =>
        s.remove(),
      ),
      Array.from(document.querySelectorAll(".comfy-menu")).forEach((s) =>
        s.remove(),
      ),
  ...
  s = async () => {
    ...
    for (let o in this.app.graph._nodes) {
      const e = this.app.graph._nodes[o],
        s = t[e.type];
      if (
        (typeof e.refreshComboInNode == "function" && e.refreshComboInNode(t),
          !!s)
      )
        for (const a in e.widgets) {
          const i = e.widgets[a];
          i.type == "combo" &&
            s.input.required[i.name] !== void 0 &&
            (i.options.values = s.input.required[i.name][0]);
        }
    }
  };
  c = (t) => {
    ...
    for (const o of t.workflow.nodes) {
      ...
      if (e.widgets)
        for (const s of e.widgets) s.afterQueued && s.afterQueued();
    }
  };
  h = (t) => {
    t.length > 0 &&
      this.e(
        "missing_nodes",
        Array.from(new Set(t.filter((o) => typeof o == "string"))),
      );
  };
  l = () => {
  ...
  apiURL(t) {
    if (t.startsWith("/view")) {
      ...
      if (o.get("filename")) {
        ...
        if (
          (o.get("subfolder") && (e = o.get("subfolder") + "/" + e),
  ...
  getExtensions = async () => {
    const t = h
      .filter((o) => !o.endsWith("impact-sam-editor.js"))
      .map((o) => "/comfy" + o);
    return t;
  };
  getEmbeddings = async () => [];
            ...
            : e[0][s] === "__inputs__"
              ...
              (t[o] = [
                [
                  ...e[0].slice(0, s),
                  ...this.inputs
                    .filter((a) => a.startsWith("embeddings/"))
                    .map((a) => a.slice(11)),
                ],
                ...e.slice(1),
              ]);
  ...
  o = () => {
    const t = this.app.graph._nodes.reduce(
      (a, i) => {
        i.pos[0] < a[0] && (a[0] = i.pos[0]),
          i.pos[1] < a[1] && (a[1] = i.pos[1]);
    ...
    this.app.canvas.setZoom(1),
      (this.app.canvas.ds.offset = new Float32Array([e, s])),
      this.app.canvas.draw(!0, !0);
  };
  getNodeDefs = async () => {
    const t = {};
    for (const [o, e] of Object.entries(l))
      if (!g.has(o))
        if (e.input) {
          let s = { ...e, input: {} };
          e.input.required &&
            (s.input.required = this.a({ ...e.input.required })),
            e.input.optional &&
            (s.input.optional = this.a({ ...e.input.optional })),
            (t[o] = s);
        } else t[o] = e;
    return t;
  };
  getItems = async (t) =>
    t === "queue" ? { Running: [], Pending: [] } : { History: [] };
