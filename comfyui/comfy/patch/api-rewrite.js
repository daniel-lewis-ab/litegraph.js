...
class ComfyGraphPatcher extends EventTarget {
    ...
    if (message.internal) {
      ...
      switch (internalMessage.type) {
        ...
        case 'executing': {
          ...
          // this.app.runningNodeID = internalMessage.data.node;
          // this.app.graph.setDirtyCanvas(true, false);
          // delete this.app.nodePreviewImages[this.runningNodeId];
          // console.log('Updating executing node ', internalMessage.data.node);
          break;
        }
        case 'executed': {
  ...
  getExtensions = async () => {
    const ext = extensions.filter((path) => !path.endsWith('impact-sam-editor.js')).map((path) => '/comfy' + path);
  ...
  createIFrameCommunication = (isReconnect) => {
    ...
    window.addEventListener('webSocketMessage', (event) => {
      try {
        ...
        //         window.name = this.clientId;
        //       }
        //       window.dispatchEvent(new CustomEvent('status', { detail: msg.data.status }));
        //       break;
        //     case 'progress':
        //       window.dispatchEvent(new CustomEvent('progress', { detail: msg.data }));
        //       break;
        //     case 'executing':
        //       window.dispatchEvent(new CustomEvent('executing', { detail: msg.data.node }));
        //       break;
        //     case 'executed':
        //       window.dispatchEvent(new CustomEvent('executed', { detail: msg.data }));
        //       break;
        //     case 'execution_start':
        //       window.dispatchEvent(new CustomEvent('execution_start', { detail: msg.data }));
        //       break;
        //     case 'execution_error':
        //       window.dispatchEvent(new CustomEvent('execution_error', { detail: msg.data }));
        //       break;
        //     case 'execution_cached':
        //       window.dispatchEvent(new CustomEvent('execution_cached', { detail: msg.data }));
        //       break;
        //     default:
